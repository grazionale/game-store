  ...
  context 'POST /system_requirements' do
    ...
    context 'with valid params' do
      ...
      it 'returns last added SystemRequirement' do
        post url, headers: auth_header(user), params: system_requirement_params
        expected_coupon = SystemRequirement.last.as_json(only: %i[id name operational_system storage processor memory video_board])
        expect(body_json['system_requirement']).to eq expected_coupon
      end
  ...
  context "PATCH /system_requirements/:id" do
    ...
    context "with valid params" do
      ...
      it 'returns updated SystemRequirement' do
        patch url, headers: auth_header(user), params: system_requirement_params
        system_requirement.reload
        expected_coupon = system_requirement.as_json(only: %i(id name operational_system storage processor memory video_board))
        expect(body_json['system_requirement']).to eq expected_coupon
      end
