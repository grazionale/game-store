shared_examples "like searchable concern" do |factory_name, field|
  ...
  let!(:records_to_find) do 
    (0..3).to_a.map { |index| create(factory_name, field => "Example #{index}") } # cria uma lisa com nomes que contém 'Example'
  end
  let!(:records_to_ignore) { create_list(factory_name, 3) } # cria uma lista com nomes aleatórios

  it "found records with expression in :#{field}" do
    # described_class para chamar a classe que está sendo testada
